...
**Dataset returns a 5-tuple** `(signal, sbp_dbp, demographics, abp_minmax, age_raw)` (see `src/data_module/cardiac_dataset.py`):
- `signal`: `(3, L_slot)` model-slot waveform (as above).
- `sbp_dbp`: `(2,)` scalar SBP/DBP labels read from a **sibling `.csv`** (same dir + stem as the `.npy`, per-cycle mean labeling — *not* per-segment min/max). Missing CSV/columns → zero-filled with a WARNING. **Used as the BP-head target only when `data.bp_label_source=per_cycle_mean` (legacy); the default `segment_minmax` instead derives the target from `abp_minmax` — see below.**
...
- `abp_minmax`: `(2,) = (dbp_seg, sbp_seg)` — the per-segment raw mmHg extrema (`abp.min()`/`abp.max()`) used for slot-2 min-max. **`evaluate.py` uses this to exactly invert the normalization and recover the raw on-disk ABP waveform as the metric target** (`reconstruct_mmHg(shape, sbp_seg, dbp_seg)` is the exact inverse). It is **also** the BP-head regression target under the default `data.bp_label_source=segment_minmax` (SBP=`sbp_seg`, DBP=`dbp_seg`, matching MD-ViSCo's "SBP/DBP = max/min of ABP"), and the raw mmHg weights for the WCL waveform terms.
- `age_raw`: `(1,)` raw age in years (NaN→population mean) — the WCL `text_age_wcl` weight (threshold 0.0235 @ `temperature_weight=4` ⇔ "age diff ≤ ~15 years", meaningful only in raw years). Ignored by every non-WCL consumer.

The **RF backbone trainer only consumes `signal`** (`batch[0]`); the other elements are ignored there (`abp_minmax` is eval-only for it). `sbp_dbp`/`demographics`/`abp_minmax`/`age_raw` feed the BP head. When `data.bp_label_norm` is set (default `{vmin: 40, vmax: 180}`), the BP-head target (whichever source) is min-max normalized to `[0, 1]` (MD-ViSCo Sec III.D); `BPLabelNorm.from_cfg(cfg.data).denormalize(...)` recovers mmHg at eval. Set it empty for raw-mmHg labels (legacy).
...
│   ├── wcl.py            # multi-WCL criterion (faithful MD-ViSCo port; 6 terms)
│   ├── bp_head_trainer.py# BP-head train loop (L1 / wcl_only / l1+wcl, single fixed task, no RF/sampler)
...
- **Model** (`src/model_module/bp_head.py`, `BPHead`, ~385M params): per source vital (ECG slot 0, PPG slot 1) an independent stack — HF `PatchTSMixerModel` waveform encoder → `ProjectionHead` → fuse with a demographics embedding as a 2nd channel → second `PatchTSMixerModel` → `MlpBP` SBP/DBP dual head. Per-vital `(SBP, DBP)` are **averaged over the active vitals**. `forward(return_embeddings=True)` / `encode_embeddings(...)` expose the per-vital waveform embeddings + demo embedding for WCL. One remaining intentional deviation from MD-ViSCo's pi=true config (deferred by the user, not a simplification): demographics come from a numeric MLP instead of DistilBERT. **WCL is now implemented** (was previously deferred) — see "BP head loss modes" below. Config: `run/conf/model/bp_head.yaml` → `BPHeadConfig`.
...
- **Training** (`run/pipeline/train_bp_head.py` → `src/trainer_module/bp_head_trainer.py`, config root `run/conf/config_bp_head.yaml` + `run/conf/trainer/bp_head.yaml`): no time embedding / mask / Euler. **Single fixed task — NOT RF-style per-batch task sampling.** The head trains on one task (`trainer.bp_task`, default `ecgppg2abp`); `cond_slots_to_vitals(bp_task)` sets `active_vitals=["ecg","ppg"]` for every step (both vitals, averaged), matching how `bp_metrics` feeds the multimodal eval cascade. Same two-stage split as the RF backbone (`stage=pretrain` Train_Subset 80/20; `stage=finetune` CalFree 80/10/10). Eval is **per-task** (over the single trained task): val/test report ME±SD in mmHg (SD is sample stddev `ddof=1`, consistent with `bp_metrics`). Scripts: `train_bp_head.sh`, `finetune_bp_head.sh`.
- **BP head loss modes** (`trainer.wcl`, resolved by `_resolve_loss_mode(wcl, stage)`): MD-ViSCo's `L_ref = L_MAE + L_WCL` is now faithfully implemented (`src/trainer_module/wcl.py` — the six-term multi-WCL: 4 waveform ECG/PPG×SBP/DBP @ scale 1e-3 + 2 PI gender/age @ 1e-2; exact ports of `weighted_contrastive_loss.py`).
  - `wcl.enabled=false` → `l1` (legacy pure L1).
  - `stage=pretrain` + `wcl.pretrain_contrastive_only=true` (default) → `wcl_only`: **self-supervised contrastive pretraining** of the encoders/projection + demo MLP (skips fusion + MlpBP heads via `encode_embeddings`); `best.pt` by min val WCL loss. This is `train_bp_head.sh` stage-1.
  - `stage=finetune` → `l1+wcl`: `best.pt` by min mean per-task val L1. This is `finetune_bp_head.sh` stage-2 (inits from the stage-1 contrastive ckpt).
  WCL waveform-term raw weights come from `abp_minmax` (raw mmHg SBP/DBP); the age term from `age_raw`; gender from `demographics[:,1]`.
- **BP-head metric target** (`data.bp_label_source`, default `segment_minmax`): the head regresses per-segment ABP max/min (from `abp_minmax`) so its ME/SD measure the **same quantity as MD-ViSCo Table III** (which reports BP point metrics in the **calibration-based** refinement setting — its refinement finetunes on a re-split of the test patients; cf. the benchmark table's finetune row). Set `per_cycle_mean` to regress PulseDB's CSV per-cycle-mean labels (legacy). The same flag drives both `bp_head_trainer` eval and `bp_metrics.evaluate_bp_test`.
